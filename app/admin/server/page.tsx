...
export default async function AdminServerPage() {
  ...
  const resourceRows = [
    ...
    { label: "Cache Storage", value: system.redis === "Configured" ? 85 : 20 },
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        <MetricCard label="Data Collection" value={system.mongo} detail="Primary CRM and activity store." tone={system.mongo === "Connected" ? "green" : "orange"} />
        <MetricCard label="Cache Storage" value={system.redis} detail="Usage counters and quota cache." tone={system.redis === "Configured" ? "green" : "orange"} />
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        ...
        <AdminPanel title="Runtime checklist" description="Useful checks before production traffic.">
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-3xl bg-[#fbfaf8] p-4"><span className="flex items-center gap-3 font-bold"><Database className="h-5 w-5 text-[#f6821f]" /> Data Collection URL</span><StatusPill tone={system.mongo === "Connected" ? "green" : "red"}>{system.mongo}</StatusPill></div>
            <div className="flex items-center justify-between rounded-3xl bg-[#fbfaf8] p-4"><span className="flex items-center gap-3 font-bold"><Wifi className="h-5 w-5 text-[#f6821f]" /> Cache Storage URL</span><StatusPill tone={system.redis === "Configured" ? "green" : "red"}>{system.redis}</StatusPill></div>
      ...
      <AdminPanel title="Suggested server pages" description="Extra admin menu ideas I would add next.">
        <div className="grid gap-4 md:grid-cols-3">
          {["Webhook abuse queue", "Cache Storage quota browser", "Background job monitor"].map((item) => (
